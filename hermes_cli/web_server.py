...
def _cron_job_update_payload(body: CronJobUpdate) -> Dict[str, Any]:
    """Normalize legacy explicit fields and upstream generic update payloads."""
    if body.updates is not None:
        return dict(body.updates)

    if body.prompt is None or body.schedule is None:
        raise HTTPException(
            status_code=400,
            detail="Cron job updates require either an updates object or prompt + schedule fields.",
        )

    from cron.jobs import parse_schedule

    schedule_text = body.schedule.strip()
    return {
        "prompt": body.prompt,
...
async def update_cron_job(job_id: str, body: CronJobUpdate, profile: Optional[str] = None):
    selected = profile or _find_cron_job_profile(job_id)
    try:
        # Legacy explicit-field edits predate profile-aware dashboard lookup and
        # tests monkeypatch cron.jobs.update_job only. Validate/parse them before
        # requiring an existing job; the underlying cron update path still decides
        # whether the id exists.
        updates = _cron_job_update_payload(body)
        if not selected:
            if profile is None and body.updates is None:
                selected = "default"
            else:
                raise HTTPException(status_code=404, detail="Job not found")
        profile_name, profile_home = _cron_profile_home(selected)
        existing = _call_cron_for_profile(profile_name, "get_job", job_id)
        if not existing and body.updates is not None:
            raise HTTPException(status_code=404, detail="Job not found")
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    try:
        ...
        wrapper_path = wrapper_dir / (f"{body.name}.bat" if is_windows else body.name)
        if not collision or not wrapper_path.exists():
