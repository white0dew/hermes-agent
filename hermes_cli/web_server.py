...
async def update_cron_job(job_id: str, body: CronJobUpdate, profile: Optional[str] = None):
    selected = profile or _find_cron_job_profile(job_id)
    if not selected:
        # Preserve the local explicit-field edit flow for legacy/default-profile
        # callers while keeping upstream 404 behaviour for generic profile-agnostic
        # update payloads.
